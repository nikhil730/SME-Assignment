...
namespace Player
{
	...
	void PlayerController::processBulletFire() {
		// Logic to handle firing bullets
		if (event_service->pressedLeftMouseButton()) {
			fireSimpleBullet(); // Call the appropriate bullet firing method
		}
	}


	void PlayerController::fireSimpleBullet() {
		// Create a new bullet object
		SimpleBullet::Bullet simpleBullet;

		// Set the bullet's position to the player's position
		sf::Vector2f playerPosition = player_model->getPlayerPosition();
		simpleBullet.setPosition(playerPosition);

		sf::Vector2f bulletVelocity(0.0f, -1.0f);
		simpleBullet.setVelocity(bulletVelocity);

		// Render the bullet on the screen
		renderBullet(simpleBullet);
